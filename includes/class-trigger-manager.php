...
class MYP_Telegram_Trigger_Manager {
	...
	private function current_user_name() {
		...
		return $user->exists()
			...
			: $this->safe_text( __( 'WordPress/System', 'telegram-trigger' ), 100 );
	}
	...
	public function send_content_alert( $post, $action, $url = null ) {
		...
		$title       = '' !== $title ? $title : __( 'Untitled', 'telegram-trigger' );
	...
	public function send_comment_alert( $comment, $action ) {
		...
		if ( $post ) {
			$title = get_the_title( $post->ID );
			$values['content_title'] = $this->safe_text( '' !== $title ? $title : __( 'Untitled', 'telegram-trigger' ), 250 );
