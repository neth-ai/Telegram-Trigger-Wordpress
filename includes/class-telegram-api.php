...
class MYP_Telegram_API {
	...
	public function send( $message, $args = array() ) {
		...
		if ( ! is_string( $message ) ) {
			$this->set_last_error( __( 'The Telegram message must be plain text.', 'telegram-trigger' ) );
		...
		if ( '' === $message ) {
			$this->set_last_error( __( 'The Telegram message is empty.', 'telegram-trigger' ) );

			return false;
		}

		if ( ! $this->is_valid_token( $token ) ) {
			$this->set_last_error( __( 'The bot token format is invalid.', 'telegram-trigger' ) );

			return false;
		}

		if ( ! $chats ) {
			$this->set_last_error( __( 'No valid Chat ID was found. Enter a numeric group or channel ID; the plugin adds one leading minus sign automatically.', 'telegram-trigger' ) );
	...
	public function send_test() {
		if ( ! myp_telegram_settings()->is_configured() ) {
			return array(
				'ok'      => false,
				'message' => __( 'Please save a valid bot token and at least one chat ID first.', 'telegram-trigger' ),
			);
		}
		...
		return array(
			'ok'      => $sent,
			'message' => $sent
				? __( 'Test message sent. Check Telegram.', 'telegram-trigger' )
				: ( '' !== $error ? $error : __( 'Telegram rejected the request. Verify the token, Chat ID, and bot permissions.', 'telegram-trigger' ) ),
		);
	}
	...
	private function send_to_chat( $token, $chat_id, $message, $parse_mode ) {
		...
		if ( is_wp_error( $response ) ) {
			$this->set_last_error( __( 'WordPress could not connect to Telegram. Check the server connection and try again.', 'telegram-trigger' ) );
	...
	private function friendly_telegram_error( $description ) {
		...
		if ( false !== strpos( $normalized, 'chat not found' ) ) {
			return __( 'Telegram could not access this group or channel. Add the bot to that chat, grant posting permission, and verify the numeric ID.', 'telegram-trigger' );
		}

		if ( false !== strpos( $normalized, 'bot was blocked' ) ) {
			return __( 'The Telegram user blocked this bot. Unblock it, open the bot, and send /start before testing again.', 'telegram-trigger' );
		}

		if ( false !== strpos( $normalized, 'not enough rights' ) || false !== strpos( $normalized, 'not a member' ) ) {
			return __( 'The bot does not have permission to post in this group or channel. Add the bot and grant permission, then test again.', 'telegram-trigger' );
		}

		if ( false !== strpos( $normalized, 'unauthorized' ) ) {
			return __( 'Telegram rejected the bot token. Copy the current token from @BotFather and save it again.', 'telegram-trigger' );
		}

		if ( '' !== $description ) {
			return sprintf(
				/* translators: %s: Error description returned by the Telegram Bot API. */
				__( 'Telegram API error: %s', 'telegram-trigger' ),
				$description
			);
		}

		return __( 'Telegram rejected the request. Verify the token, Chat ID, and bot permissions.', 'telegram-trigger' );
	}
