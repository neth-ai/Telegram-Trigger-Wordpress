...
<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" class="myp-form">
	...
	<section class="myp-card myp-format-help">
		<div class="myp-card__header">
			<div>
				<div class="myp-card__title"><?php esc_html_e( 'How message formats work', 'telegram-bot-trigger-notifications' ); ?></div>
				<p class="myp-card__desc"><?php esc_html_e( 'Write labels in any language and place the available dynamic values wherever you want. A complete line is removed automatically when its optional value is empty.', 'telegram-bot-trigger-notifications' ); ?></p>
			</div>
		</div>
		<p class="myp-format-help__example"><code>សកម្មភាព: {action_text}</code> → <span><?php esc_html_e( 'The Khmer label stays fixed while the action value changes for each event.', 'telegram-bot-trigger-notifications' ); ?></span></p>
	</section>
			...
			<div class="myp-format-layout">
				<div class="myp-format-editor">
					<label class="myp-field myp-format-icon">
						<span><?php esc_html_e( 'Icon or emoji', 'telegram-bot-trigger-notifications' ); ?></span>
						...
						<small><?php esc_html_e( 'One or more emoji are allowed. Leave empty if the title already contains an icon.', 'telegram-bot-trigger-notifications' ); ?></small>
					</label>

					<label class="myp-field">
						<span><?php esc_html_e( 'Message template', 'telegram-bot-trigger-notifications' ); ?></span>
					...
					<div class="myp-format-placeholders" aria-label="<?php esc_attr_e( 'Available placeholders', 'telegram-bot-trigger-notifications' ); ?>">
						<span><?php esc_html_e( 'Insert value:', 'telegram-bot-trigger-notifications' ); ?></span>
					...
					<?php if ( ! empty( $definition['show_role'] ) ) : ?>
						<div class="myp-setting-row myp-setting-row--nested myp-format-option">
							...
							<div class="myp-setting-row__copy">
								<strong><?php esc_html_e( 'Show role', 'telegram-bot-trigger-notifications' ); ?></strong>
								<span><?php esc_html_e( 'Turn off to remove the complete line containing {role}.', 'telegram-bot-trigger-notifications' ); ?></span>
							</div>
							<label class="myp-switch">
								...
								<span class="screen-reader-text"><?php esc_html_e( 'Show role', 'telegram-bot-trigger-notifications' ); ?></span>
							</label>
						</div>
					<?php endif; ?>
				</div>
				...
				<div class="myp-format-preview">
					<span><?php esc_html_e( 'Telegram preview', 'telegram-bot-trigger-notifications' ); ?></span>
	...
	<div class="myp-action-bar">
		<div>
			<strong><?php esc_html_e( 'Apply message formats', 'telegram-bot-trigger-notifications' ); ?></strong>
			<span><?php esc_html_e( 'New Telegram notifications will use these icons, labels, fields, and placeholders.', 'telegram-bot-trigger-notifications' ); ?></span>
		</div>
		<button type="submit" class="button button-primary myp-button">
			<span class="dashicons dashicons-saved" aria-hidden="true"></span>
			<?php esc_html_e( 'Save message formats', 'telegram-bot-trigger-notifications' ); ?>
		</button>
	</div>
</form>
</div>
